...
class OpenEOGateway:
    ...
    async def execute_sync(self, process_graph: dict) -> bytes:
        ...
        native_tf = props.get("earthgrid:transform")  # [a, b, c, d, e, f] from ingest
        bbox   = reference_item.bbox  # [west, south, east, north] — STAC bbox is always WGS84

        from rasterio.transform import Affine, from_bounds

        # Prefer stored native transform (exact pixel grid from original COG)
        if native_tf and len(native_tf) == 6:
            transform = Affine(*native_tf)
            crs = native_crs
        else:
            # Fallback: compute from bbox (less precise for projected CRS)
