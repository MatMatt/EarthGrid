...
async fn ui_page(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let cols_options: String = collections.iter().map(|c| {
        format!(r#"<option value="{}">{}</option>"#, c.id, c.id)
    }).collect::<Vec<_>>().join("\n");
...
nav{{background:#111;padding:12px 20px;border-bottom:1px solid #222;display:flex;align-items:center;gap:12px}}
nav h2{{margin:0;color:#4fc3f7}}
...
input,select{{background:#0d0d1a;color:#e0e0e0;border:1px solid #333;border-radius:4px;padding:6px 10px;font-size:0.9em}}
button,.btn{{background:#4fc3f7;color:#000;border:none;border-radius:4px;padding:8px 16px;cursor:pointer;font-size:0.9em;font-weight:600}}
...
.tabs{{display:flex;gap:0;margin:16px 0 0;border-bottom:2px solid #222}}
.tab{{padding:10px 20px;cursor:pointer;color:#888;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .2s}}
...
.stat-box{{background:#0d0d1a;border:1px solid #1a1a2e;border-radius:6px;padding:14px}}
.stat-box .sv{{font-size:1.4em;font-weight:bold;color:#4fc3f7}}
.stat-box .sl{{color:#888;font-size:0.8em;margin-top:2px}}
.form-row{{display:flex;gap:8px;align-items:center;margin:8px 0;flex-wrap:wrap}}
.form-row label{{color:#888;font-size:0.85em;min-width:80px}}
.form-row input{{flex:1;min-width:100px}}
.status-msg{{padding:8px 12px;border-radius:4px;margin:8px 0;font-size:0.9em}}
.status-ok{{background:#1b5e2033;border:1px solid #2e7d32;color:#81c784}}
.status-err{{background:#b7161633;border:1px solid #c62828;color:#ef9a9a}}
.empty{{color:#666;font-style:italic;padding:20px;text-align:center}}
.peer-status{{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}}
.peer-online{{background:#4caf50}}
.peer-stale{{background:#ff9800}}
tr:hover td{{background:#0d0d1a22}}
...
<nav>
  <h2>🌍 EarthGrid</h2>
  <span style="color:#666;font-size:0.8em">v{version}</span>
</nav>
<main>
<div class="card">
  <h3 style="margin-top:0">Node: {node_name}</h3>
...
<div class="tabs">
  ...
  <div class="tab" onclick="switchTab('fetch')">🛰️ Fetch</div>
</div>
...
<div id="tab-stats" class="panel active">
  <div class="card">
    ...
    <div style="text-align:right;margin-top:8px;color:#555;font-size:0.75em">
      Auto-refreshes every 30s · <span id="stats-updated"></span>
    </div>
  </div>
</div>

<!-- Peers Panel -->
<div id="tab-peers" class="panel">
  <div class="card">
    <div id="peers-content"><div class="empty">Loading peers...</div></div>
    <div style="text-align:right;margin-top:8px;color:#555;font-size:0.75em">
      Auto-refreshes every 30s · <span id="peers-updated"></span>
    </div>
  </div>
</div>

<!-- Collections Panel -->
<div id="tab-collections" class="panel">
  <div class="card">
...
<div id="tab-audit" class="panel">
  <div class="card">
    <div id="audit-content"><div class="empty">Loading audit log...</div></div>
  </div>
</div>
...
<div id="tab-manage" class="panel">
  <div class="card">
    ...
    <div id="config-display" class="stat-grid" style="margin-bottom:16px"></div>

    <h4>Rename Node</h4>
    <div class="form-row">
      <input type="text" id="new-name" placeholder="New node name" value="{node_name}">
      <button class="btn-sm" onclick="renameNode()">Rename</button>
    </div>
    <div id="rename-status"></div>

    <h4>Resize Storage</h4>
    <div class="form-row">
      <input type="number" id="new-size" placeholder="Size in GB" min="1">
      <span style="color:#888">GB</span>
      <button class="btn-sm" onclick="resizeStorage()">Resize</button>
    </div>
...
<!-- Fetch Panel -->
<div id="tab-fetch" class="panel">
  <div class="card">
    <h3 style="margin-top:0">Fetch Satellite Data</h3>
    <div class="form-row">
      <label>Collection</label>
      <select id="fetch-collection">
        {cols_options}
      </select>
    </div>
    <div class="form-row">
      <label>BBox</label>
      <input type="number" step="0.01" id="bbox-west" placeholder="West" style="flex:1">
      <input type="number" step="0.01" id="bbox-south" placeholder="South" style="flex:1">
      <input type="number" step="0.01" id="bbox-east" placeholder="East" style="flex:1">
      <input type="number" step="0.01" id="bbox-north" placeholder="North" style="flex:1">
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
      ...
      <input type="number" id="fetch-cloud" placeholder="Max %" value="30" min="0" max="100" style="width:80px">
      <label style="min-width:40px">Limit</label>
      <input type="number" id="fetch-limit" placeholder="Max items" value="10" min="1" style="width:80px">
    </div>
    <div class="form-row">
      <button onclick="fetchData()">🛰️ Start Fetch</button>
    </div>
...
<p style="padding:0 20px;color:#333;font-size:0.75em">{node_id}</p>
...
async function loadStats() {{
  try {{
    ...
    const uptime=Date.now()/1000 - d.started;
    const h=Math.floor(uptime/3600);
    const m=Math.floor((uptime%3600)/60);
    document.getElementById('stats-content').innerHTML=`
      <div class="stat-grid">
        <div class="stat-box"><div class="sv">${{fmtBytes(d.bytes_ingested)}}</div><div class="sl">Bytes Ingested</div></div>
        <div class="stat-box"><div class="sv">${{fmtBytes(d.bytes_served)}}</div><div class="sl">Bytes Served</div></div>
        <div class="stat-box"><div class="sv">${{d.total_chunks.toLocaleString()}}</div><div class="sl">Chunks Stored</div></div>
        <div class="stat-box"><div class="sv">${{d.chunks_served.toLocaleString()}}</div><div class="sl">Chunks Served</div></div>
        <div class="stat-box"><div class="sv">${{d.requests_total.toLocaleString()}}</div><div class="sl">Total Requests</div></div>
        <div class="stat-box"><div class="sv">${{fmtBytes(d.total_bytes)}}</div><div class="sl">Total Storage</div></div>
        <div class="stat-box"><div class="sv">${{h}}h ${{m}}m</div><div class="sl">Uptime</div></div>
        <div class="stat-box"><div class="sv">${{fmtTime(d.started)}}</div><div class="sl">Started</div></div>
      </div>`;
    document.getElementById('stats-updated').textContent='Updated '+new Date().toLocaleTimeString();
  }} catch(e) {{
    document.getElementById('stats-content').innerHTML='<div class="status-msg status-err">Failed to load stats</div>';
  }}
}}
  ...
  try {{
    ...
    if(!d.peers||d.peers.length===0) {{
      document.getElementById('peers-content').innerHTML=`
        <div class="empty">No peers connected<br><span style="font-size:0.85em;color:#555">Other nodes will appear here when they join the network</span></div>`;
    }} else {{
      const rows=d.peers.map(p => {{
        const ago=fmtAgo(p.last_seen);
        const stale=(Date.now()/1000 - p.last_seen) > 300;
        return `<tr>
          <td><span class="peer-status ${{stale?'peer-stale':'peer-online'}}"></span>${{p.node_name||'unnamed'}}</td>
          <td class="mono">${{(p.node_id||'').substring(0,12)}}...</td>
          <td class="mono">${{p.url||'-'}}</td>
          <td>${{ago}}</td>
        </tr>`;
      }}).join('');
      document.getElementById('peers-content').innerHTML=`
        <div style="margin-bottom:12px;color:#888">${{d.peers.length}} peer(s) connected</div>
        <table>
          <thead><tr><th>Node</th><th>ID</th><th>URL</th><th>Last Seen</th></tr></thead>
          <tbody>${{rows}}</tbody>
        </table>`;
    }}
    document.getElementById('peers-updated').textContent='Updated '+new Date().toLocaleTimeString();
  }} catch(e) {{
    document.getElementById('peers-content').innerHTML='<div class="status-msg status-err">Failed to load peers</div>';
  }}
}}

async function loadAudit() {{
  try {{
    const r=await fetch('/audit?limit=30');
    const d=await r.json();
    const entries=d.entries||d||[];
    if(entries.length===0) {{
      document.getElementById('audit-content').innerHTML='<div class="empty">No audit entries yet</div>';
      return;
    }}
    const rows=entries.map(e => `<tr>
      <td style="white-space:nowrap">${{e.timestamp?fmtTime(e.timestamp):(e.time||'')}}</td>
      <td><strong>${{e.action||e.event||''}}</strong></td>
      <td style="color:#888;font-size:0.85em">${{e.details||e.message||JSON.stringify(e).substring(0,120)}}</td>
    </tr>`).join('');
    document.getElementById('audit-content').innerHTML=`
      <h3 style="margin-top:0">Audit Log <span style="color:#666;font-weight:normal;font-size:0.7em">(last 30)</span></h3>
      <table>
        <thead><tr><th>Time</th><th>Action</th><th>Details</th></tr></thead>
        <tbody>${{rows}}</tbody>
      </table>`;
  }} catch(e) {{
    document.getElementById('audit-content').innerHTML='<div class="status-msg status-err">Failed to load audit log</div>';
  }}
}}
...
async function loadConfig() {{
  try {{
    ...
    document.getElementById('config-display').innerHTML=`
      <div class="stat-box"><div class="sv">${{d.node_name}}</div><div class="sl">Node Name</div></div>
      <div class="stat-box"><div class="sv">${{d.storage_gb?.toFixed(1)||'?'}} GB</div><div class="sl">Storage Used</div></div>
      <div class="stat-box"><div class="sv">${{d.items}}</div><div class="sl">Items</div></div>
      <div class="stat-box"><div class="sv">${{d.auth_enabled?'Yes':'No'}}</div><div class="sl">Auth Enabled</div></div>`;
  }} catch(e) {{}}
}}
...
async function renameNode() {{
  ...
  if(!name) return;
  try {{
    const r=await fetch('/admin/node/name',{{method:'PATCH',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{name:name}})}});
    const d=await r.json();
    document.getElementById('rename-status').innerHTML=r.ok?
      '<div class="status-msg status-ok">Node renamed! Reload to see changes.</div>':
      '<div class="status-msg status-err">'+JSON.stringify(d)+'</div>';
  }} catch(e) {{
    document.getElementById('rename-status').innerHTML='<div class="status-msg status-err">Request failed</div>';
  }}
}}

async function resizeStorage() {{
  const gb=parseInt(document.getElementById('new-size').value);
  if(!gb||gb<1) return;
  try {{
    const r=await fetch('/admin/resize',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{size_gb:gb}})}});
    const d=await r.json();
    document.getElementById('resize-status').innerHTML=r.ok?
      '<div class="status-msg status-ok">Storage resized to '+gb+' GB</div>':
      '<div class="status-msg status-err">'+JSON.stringify(d)+'</div>';
  }} catch(e) {{
    document.getElementById('resize-status').innerHTML='<div class="status-msg status-err">Request failed</div>';
  }}
}}
...
async function fetchData() {{
  ...
  if(!w||!s||!e||!n) {{
    document.getElementById('fetch-status').innerHTML='<div class="status-msg status-err">Please enter a bounding box</div>';
    return;
  }}
  const url=`/fetch?bbox=${{w}},${{s}},${{e}},${{n}}&collection=${{col}}&start_date=${{start||'2020-01-01'}}&end_date=${{end||new Date().toISOString().split('T')[0]}}&cloud_cover=${{cloud}}&limit=${{limit}}`;
  document.getElementById('fetch-status').innerHTML='<div class="status-msg status-ok">⏳ Fetching... this may take a while</div>';
  try {{
    const r=await fetch(url);
    const d=await r.json();
    const msg=d.message||d.error||JSON.stringify(d).substring(0,200);
    document.getElementById('fetch-status').innerHTML=r.ok?
      '<div class="status-msg status-ok">✅ '+msg+'</div>':
      '<div class="status-msg status-err">'+msg+'</div>';
  }} catch(err) {{
    document.getElementById('fetch-status').innerHTML='<div class="status-msg status-err">Request failed: '+err.message+'</div>';
  }}
}}
